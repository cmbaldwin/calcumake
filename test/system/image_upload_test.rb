...
class ImageUploadTest < ApplicationSystemTestCase
  ...
  test "file input accepts only images" do
    ...
    file_input = find('input[type="file"][accept="image/*"]', visible: :all)
    assert_equal "image/*", file_input["accept"]
  end
  ...
  test "error message container exists" do
    ...
    within '[data-controller="image-upload"]' do
      assert_selector '[data-image-upload-target="error"]', visible: :hidden
    end
  end
